...
<head>
    <meta charset="utf-8">
    <title>{{ $exportData['headings'] }} - PriceFeed</title>
    <style>
        .clearfix:after {
            content: "";
            display: table;
    ...
    <table>
        ...
        <tr>
            @foreach($exportData['datasets'][0] as $index => $heading)
                <th style="text-align: @if($index < 1) left @else center @endif">{{ $heading }}</th>
            @endforeach
        </tr>
        </thead>
        <tbody>
        ...
        @foreach(array_slice($exportData['datasets'], 1) as $row)
            <tr>
                @foreach($row as $index => $data)
                    <td style="text-align: @if($index < 1) left @else center @endif">
                        {{ $data }}
                    </td>
                @endforeach
            </tr>
