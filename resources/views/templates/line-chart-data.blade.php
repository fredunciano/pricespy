...
<header class="clearfix">
    ...
    <h1>Category Price Comparison Report</h1>
    <ul id="project">
        <li>{{ auth()->user()->name }}</li>
        <li>{{ auth()->user()->company }}</li>
        <li style="padding-bottom: 10px"><a href="mailto:{{ auth()->user()->email }}">{{ auth()->user()->email }}</a></li>
        <li style="border-top: 1px solid #ffffff00;padding-top: 10px">
            A Table Comparing My Shop's Average Price per Category Compared to the Average Price Offered by the
            Competitors
        </li>
...
<main>
    <table>
        <thead>
        <tr>
            @foreach($exportData['headings'] as $index => $heading)
        ...
        @foreach($exportData['rowsData'] as $row)
            <tr>
