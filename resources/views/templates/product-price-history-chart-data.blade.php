...
<head>
    <meta charset="utf-8">
    <title>Category Price Comparison Report - PriceFeed</title>
    <style>
        .clearfix:after {
            content: "";
            display: table;
...
<header class="clearfix">
    ...
    <ul id="project">
        ...
        <li style="border-top: 1px solid #ffffff00;padding-top: 10px">
            A Table Comparing My Shop's Product Price Compared to the Competitors Product Price
        </li>
        <li>Total Product Linked: {{ $exportData['linked'] }}</li>
...
<main>
    <table>
        <thead>
        <tr>
            @foreach($exportData['datasets'][0] as $index => $heading)
                <th style="text-align: @if($index < 2) left @else center @endif">{{ $heading }}</th>
            @endforeach
        </tr>
        </thead>
        <tbody>

        @foreach(array_slice($exportData['datasets'], 1) as $row)
            <tr>
                @foreach($row as $index => $data)
                    <td style="text-align: @if($index < 2) left @else center @endif">
                        {{ $data }}
                    </td>
                @endforeach
            </tr>
